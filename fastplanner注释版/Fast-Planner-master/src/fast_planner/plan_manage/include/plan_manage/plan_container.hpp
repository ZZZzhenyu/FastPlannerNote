...
public:
  ...
  void setGlobalTraj(const PolynomialTraj& traj, const ros::Time& time) {//多项式轨迹
    global_traj_ = traj;
    global_traj_.init();//得到轨迹段数num_seg和总时间time_sum
  ...
  void setLocalTraj(NonUniformBspline traj, double local_ts, double local_te, double time_inc) {//非均匀bspline轨迹
