...
void newCallback(std_msgs::Empty msg) {
  traj_cmd_.clear();//计算轨迹
  traj_real_.clear();//真实轨迹
}
...
void cmdCallback(const ros::TimerEvent& e) {
  ...
  if (t_cur < traj_duration_ && t_cur >= 0.0) {
    pos = traj_[0].evaluateDeBoorT(t_cur);//traj_:传来的bspline轨迹
...
int main(int argc, char** argv) {
  ...
  ros::Subscriber bspline_sub = node.subscribe("planning/bspline", 10, bsplineCallback);//获得traj_：bspline控制点（p v a yaw dyaw）（xyz）；traj_duration_：轨迹总时间
  ros::Subscriber replan_sub = node.subscribe("planning/replan", 10, replanCallback);//重设traj_duration_
  ros::Subscriber new_sub = node.subscribe("planning/new", 10, newCallback);//清空traj_cmd_（规划的轨迹）和traj_real_（真实）
  ros::Subscriber odom_sub = node.subscribe("/odom_world", 50, odomCallbck);//根据dodmetry信息得到traj_real_（只有位置信息）
  ...
  //1、根据traj_获得cmd（p v a yaw dyaw ）（xyz），并发布pos_cmd_pub（cmd）；2、traj_cmd_赋值为cmd（p）3、调用drawCmd进行cmd_vis_pub
  ros::Timer vis_timer = node.createTimer(ros::Duration(0.25), visCallback);//调用displayTrajWithColor根据traj_cmd_的p进行traj_pub
