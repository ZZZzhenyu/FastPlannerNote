...
void FastPlannerManager::initPlanModules(ros::NodeHandle& nh) {
  //这部分除了设置ROS nh句柄空间下的参数外，最重要的是将kino_path_finder以及Bspline_optimizers进行初始化。
  /* read algorithm parameters */

  nh.param("manager/max_vel", pp_.max_vel_, -1.0);//pp_：PlanParameters
...
bool FastPlannerManager::checkTrajCollision(double& distance) {
  ...
  并根据SDF类的evaluateCorseEDT（）函数计算距离障碍物的距离：distance。如果轨迹中的任一点距离障碍物的距离小于阈值，
  则返回当前局部轨迹从起始点到检查到碰撞点的距离（即最远可执行的安全距离）。*/


  double t_now = (ros::Time::now() - local_data_.start_time_).toSec();//局部轨迹上的现在时间

  double tm, tmp;//tm:轨迹起始时间；tmp：轨迹终止时间
  local_data_.position_traj_.getTimeSpan(tm, tmp);
  Eigen::Vector3d cur_pt = local_data_.position_traj_.evaluateDeBoor(tm + t_now);//根据时间算出bspline轨迹上当前点的位置

  double          radius = 0.0;
  Eigen::Vector3d fut_pt;
  double          fut_t = 0.02;//离散步长

  while (radius < 6.0 && t_now + fut_t < local_data_.duration_) {
    fut_pt = local_data_.position_traj_.evaluateDeBoor(tm + t_now + fut_t);//离散时间点对应位置

    double dist = edt_environment_->evaluateCoarseEDT(fut_pt, -1.0);//计算距离障碍物的距离
    if (dist < 0.1) {
      distance = radius;//当前局部轨迹从起始点到检查到碰撞点的距离（即最远可执行的安全距离）
      return false;
    }
...
//核心函数，课程内容全在这里
/*把起止点状态都设置好后，就利用kino_path_finder的search函数进行路径寻找。
...
//起止点设置
  ...
  kino_path_finder_->reset();//kino_path_finder_：FastPlannerManager类下KinodynamicAstar智能指针

  int status = kino_path_finder_->search(start_pt, start_vel, start_acc, end_pt, end_vel, true);
  //search！！！！！前端轨迹搜索；最开始会以start_acc作为起始点输入进行查找，如果找不到，则再进行一轮离散化输入的寻找，若都找不到，则路径寻找失败。
  ...
  if (status == KinodynamicAstar::NO_PATH) {
    ...
    status = kino_path_finder_->search(start_pt, start_vel, start_acc, end_pt, end_vel, false);//init置false有什么作用？？？？？？？？？？
  ...
  plan_data_.kino_path_ = kino_path_finder_->getKinoTraj(0.01);//前端搜索产生的轨迹：kino_path_//返回的是路径节点，高分辨率

  t_search = (ros::Time::now() - t1).toSec();

  // parameterize the path to bspline

  double                  ts = pp_.ctrl_pt_dist / pp_.max_vel_;//轨迹的分段时间初值
  vector<Eigen::Vector3d> point_set, start_end_derivatives;
  kino_path_finder_->getSamples(ts, point_set, start_end_derivatives);//一条多项式轨迹离散化，得到离散点point_set ，返回start_end_derivatives()，ts变为每段轨迹的实际时间（均匀bspline，时间都一样）

  Eigen::MatrixXd ctrl_pts;
  NonUniformBspline::parameterizeToBspline(ts, point_set, start_end_derivatives, ctrl_pts);//对所找到的路径进行均匀B样条函数的拟合，然后得到相应控制点ctrl_pts。做为轨迹优化初值
  ...
  if (status != KinodynamicAstar::REACH_END) {//在当前轨迹只是达到感知距离以外并未达到目标点时，目标函数需要加上ENDPOINT优化项，
    cost_function |= BsplineOptimizer::ENDPOINT;
  }

  ctrl_pts = bspline_optimizers_[0]->BsplineOptimizeTraj(ctrl_pts, ts, cost_function, 1, 1);//bspline优化接口，返回优化后的控制点
  ...
  t1                    = ros::Time::now();
  NonUniformBspline pos = NonUniformBspline(ctrl_pts, 3, ts);//构建非均匀bspline轨迹对象pos，方便进行时间分配
  ...
  while (!feasible && ros::ok()) {
    ...
    feasible = pos.reallocateTime();//时间重分配函数//返回值表示每个控制点的va是否超出限制

    if (++iter_num >= 3) break;//优化次数大于3结束
  }

  // pos.checkFeasibility(true);
  // cout << "[Main]: iter num: " << iter_num << endl;

  double tn = pos.getTimeSum();//重分配后的轨迹总时间

  cout << "[kino replan]: Reallocate ratio: " << tn / to << endl;
  if (tn / to > 3.0) ROS_ERROR("reallocate error.");

  t_adjust = (ros::Time::now() - t1).toSec();//时间重分配花费的时间

  // save planned results
//将调整后的B样条轨迹赋值给local_data.position_traj. 并利用updateTrajInfo函数对local_data的其他数据进行更新。
...
// !SECTION
/*把现在规划出来的轨迹进行线性分段，分段的方法是根据轨迹的总运行时间/人为设定的时间增量，进而得到单位时间增量下的，
最小航向角增量dt_yaw
...
void FastPlannerManager::planYaw(const Eigen::Vector3d& start_yaw) {
  ...
  通过人为设定的时间增量不断迭代，取出对应时刻轨迹上的控制点，进而得到轨迹上两两相邻的控制点，通过两两控制点的相对位置即可计算得到该条轨迹上每个控制点的航向角yaw，
