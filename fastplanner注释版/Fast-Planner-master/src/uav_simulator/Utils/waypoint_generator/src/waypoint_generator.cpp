...
void load_seg(ros::NodeHandle& nh, int segid, const ros::Time& time_base) {
    /*根据设定的yaw ptx pty ptz以及odom中的位置信息，路径位置信息path_msg存入waypointSegments*/
    std::string seg_str = boost::str(bfmt("seg%d/") % segid);
    double yaw;
    double time_from_start;
    ROS_INFO("Getting segment %d", segid);//segid第几段目标轨迹
    ROS_ASSERT(nh.getParam(seg_str + "yaw", yaw));//ROS_ASSERT()不成立则中断程序
...
void load_waypoints(ros::NodeHandle& nh, const ros::Time& time_base) {//循环调用load_seg进行轨迹加载
    int seg_cnt = 0;
...
void odom_callback(const nav_msgs::Odometry::ConstPtr& msg) {
    ...
    if (waypointSegments.size()) {
        ...
        if (odom.header.stamp >= expected_time) {
            waypoints = waypointSegments.front();//返回对队列第一个元素的引用。
            ...
            waypointSegments.pop_front();//pop_front()用于从列表容器的开头弹出（删除）元素
        }
    }
}
    ...
    if (waypoint_type == string("circle")) {
        ...
        if (msg->pose.position.z > 0) {
            ...
            if (waypoints.poses.size() >= 1) {
                waypoints.poses.erase(std::prev(waypoints.poses.end()));//prev()迭代器左移一个单位
            }
            publish_waypoints_vis();
        } else {
...
int main(int argc, char** argv) {
    ...
    ros::NodeHandle n("~");
    n.param("waypoint_type", waypoint_type, string("manual"));
    ros::Subscriber sub1 = n.subscribe("odom", 10, odom_callback);//waypoints为waypointSegments第一个元素
    ros::Subscriber sub2 = n.subscribe("goal", 10, goal_callback);//1、根据waypoint_type不同，waypoints不同2、series模式下进行load_waypoints
    ros::Subscriber sub3 = n.subscribe("traj_start_trigger", 10, traj_start_trigger_callback);
    pub1 = n.advertise<nav_msgs::Path>("waypoints", 50);//调用publish_waypoints进行pub1：发布waypoints
    pub2 = n.advertise<geometry_msgs::PoseArray>("waypoints_vis", 10);//调用publish_waypoints_vis发布pos_array,只包含waypoints p信息
